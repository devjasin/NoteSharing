...
const SingleNote = async ({params}:{params:{note11:string}}) => {
  let note:Note|null=null
 try {
  const response=await fetch(`${process.env.BACKEND_URL}/notes/${params.note11}`)
  if(!response.ok){
        ...
        <div className="flex flex-col md:flex-row -mx-4">
            <div className="md:flex-1 px-4">
                <div className="h-[460px] rounded-lg bg-gray-300 dark:bg-gray-700 mb-4">
                  <Image src={note?.file as string} alt={note?.title as string} width={400} height={100} />
                </div>
